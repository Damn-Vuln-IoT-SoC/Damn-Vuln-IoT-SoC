...
def litex_setup_install_repos(config="standard", user_mode=False):
    ...
    for name in install_configs[config]:
        ...
        if repo.develop:
            ...
            os.chdir(os.path.join(current_path, name))
            if name == "migen":
                subprocess.check_call("\"{python3}\" -m pip install . {options}".format(
                    python3 = sys.executable,
...
# Modified modules install -------------------------------------------------------------------------

def install_modules(name):
    print_status(f"Installing {name} Modified Module...")
    subprocess.check_call("\"{python3}\" -m pip install --editable ../{name}/".format(
        python3 = sys.executable,
        name = name,
    ), shell=True)       
...
def main():
    ...
    if args.install:
        ...
        install_modules("litex")
        install_modules("litex-boards")
        install_modules("pythondata-cpu-vexriscv")
